...
if dashboard == "Live Revenue":
    ...
    with col5:
        st.plotly_chart(px.bar(df, x="city", y="price", title="City Revenue"), use_container_width=True)

    with col6:
        st.plotly_chart(px.pie(df, names="product", title="Product Mix"), use_container_width=True)
...
if dashboard == "Hospital ER":
    ...
    cursor.execute("INSERT INTO hospital VALUES (?,?,?,?,?)", generate_patient())
    conn.commit()
    ...
    with col6:
        st.plotly_chart(px.histogram(df, x="triage", title="Triage Distribution"), use_container_width=True)
...
if st.sidebar.button("Export Sales CSV"):
    pd.read_sql("SELECT * FROM sales", conn).to_csv("sales_export.csv", index=False)
    st.sidebar.success("Exported")

if st.sidebar.button("Export Hospital CSV"):
    pd.read_sql("SELECT * FROM hospital", conn).to_csv("hospital_export.csv", index=False)
    st.sidebar.success("Exported")
